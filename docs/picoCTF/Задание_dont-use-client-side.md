**Задание dont-use-client-side**

Переходим по ссылке и видим поле ввода пароля (рис 1)\
![](media/Задание_dont-use-client-side/media/image1.png)

Рисунок 1

Правой кнопкой мыши -&gt; смотрим исходный код (рис 2)

![img](media/Задание_dont-use-client-side/media/image2.png)

Собираем флаг (рис 3, 4)

![img](media/Задание_dont-use-client-side/media/image3.png)

![img](media/Задание_dont-use-client-side/media/image4.png)

**ОТВЕТ: picoCTF\{no_clients_plz_1a3c89\}**

